  	...
  	// setStroke obrys konturu kszta³t
       ...
       second.setStrokeLineCap(StrokeLineCap.ROUND);
        ...
        SecondCol.setOnEditCommit(
            new EventHandler<CellEditEvent<Alarm, String>>() {
                @Override
                public void handle(CellEditEvent<Alarm, String> t) { 	//zdarzenie wywo³ywane gdy 
                    ((Alarm) t.getTableView().getItems().get(		//u¿ytkownik wykonuje edycje w 
                        t.getTablePosition().getRow())			//komórce tabeli
